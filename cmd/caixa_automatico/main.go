	...
	for {
		...
		case "1":
			...
			success, err := client.RetryOperation(func() (bool, error) {
				...
				return client.Saque(numeroConta, valor, idTransacao)
			}, 3)

			if err != nil {
				fmt.Printf("Erro ao realizar saque: %v\n", err)
			} else if success {
				fmt.Printf("Saque de %.2f realizado com sucesso na conta %d (ID da transação: %s)\n", valor, numeroConta, idTransacao)
			}
